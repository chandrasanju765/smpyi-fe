    ...
    >
      ...
      >
        ...
        <img
          ...
          onError={(e) => {
            ...
            if (el.src !== blog.image) el.src = blog.image;
          }}
        />
        <span className="absolute top-3 left-3 bg-[#1D4ED8] text-white text-[10px] font-bold px-2.5 py-1 rounded-md tracking-wide uppercase">
          Featured Article
        </span>
        <span className="absolute bottom-3 left-3 flex items-center gap-1 bg-black/50 text-white text-[10px] px-2.5 py-1 rounded-full">
        ...
        <span className="absolute bottom-3 right-3 flex items-center gap-1 bg-black/50 text-white text-[10px] px-2.5 py-1 rounded-full">
      ...
      <div className="flex-1 flex flex-col justify-between gap-3 p-5 sm:p-6">
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {/* Title */}
          <h2 className="font-poppins font-bold text-[#0F172A] dark:text-[#F1F5F9] leading-snug m-0"
          ...
          <div className="flex items-center gap-1.5 text-[12px] text-[#475569] dark:text-[#CBD5E1] flex-wrap">
            {blog.authorAvatar && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={blog.authorAvatar} alt={blog.author} className="w-6 h-6 rounded-full object-cover ring-2 ring-[#F97316]/30" />
            )}
        ...
        <div className="flex flex-col gap-2.5">
          ...
          <div className="flex items-center justify-between gap-2">
            <Link
              href={`/blog/${blog.slug}`}
              className="inline-flex items-center gap-1.5 px-4 py-2 border-[1.5px] border-[#F97316] text-[#F97316] font-semibold text-[13px] rounded-lg transition-all duration-200 hover:bg-[#F97316] hover:text-white"
            >
              Read Full Article →
            </Link>
